...
function loadFilterActions() {
  ...
  const state = {
    data: {
      playlists: {
        ...
        b: { id: 'b', order: 1 },
        c: { id: 'c', order: 2 },
        a1: { id: 'a1', order: 3, parentId: 'a' },
        a2: { id: 'a2', order: 4, parentId: 'a' }
      }
    },
  ...
  const app = {
    ...
    state,
    api: { isVisibleElement: () => true },
    pages: {}
  };
  const context = {
    console,
    document: { querySelectorAll: () => [browse] },
    __SYP_CONTENT__: app
  };
  ...
  const source = fs.readFileSync(filename, 'utf8').replace(
    '  pages.subscriptions = {',
    '  app.__filterTest = { applyFilter, applySubscriptionsFilterPreference, checkedSubgroupIdsOf, getCurrentSubscriptionsPreference, normalizeSubscriptionsFilterPreference, setIncludeSubscriptionsFilter, setWholeGroupInclude, toggleIncludedSubgroup, toggleSubscriptionsGroup };\n\n  pages.subscriptions = {'
  );
  vm.runInNewContext(source, context, { filename });
  return { cards, state, ...app.__filterTest };
}
